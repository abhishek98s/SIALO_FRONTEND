...
export default function UserPostBox() {
  ...
  const [user_inputted_image_url, setUser_inputted_image_url] = useState<
    string | null
  >(null);
  ...
  const openCamera = async () => {
    if (!isCameraAccessGranted) {
      toast.error("Camera access required!", toast_info_option);
      return;
    }
    ...
    try {
      ...
      toast.error("Error accessing the camera!", toast_info_option);
    }
  };
  ...
  return (
    <>
      <CameraModel
        user_inputted_image_url={user_inputted_image_url}
      ...
      >
        <div className="top flex gap-[12px] mb-[16px]">
          <Image
            src={auth_user_image ? auth_user_image : "/icons/icon-user.svg"}
            alt="user"
            width={40}
            height={40}
            className="rounded-full border-primary-60 max-w-[40px] w-full"
          />
          <input
            type="text"
            value={caption ?? ""}
            className="bg-neutral-88 border-neutral-86 px-[16px] py-[8px]"
            onChange={handleChange}
            name="input_caption"
            placeholder="Mind writing something?"
          />
        </div>
          ...
          <div className="action flex gap-[4px]">
            ...
            <div className="relative w-[40px] h-[40px] flex-center ">
              <input
                ...
                name="image_select"
                className={`${styles.image_input} absolute z-0 border-neutral-90 outline-none`}
                id="image-input"
                accept=".png,.jpg,.jpeg"
              />
              <label
                htmlFor="image-input"
                className="absolute z-5 rounded-full w-full h-full flex-center cursor-pointer bg-neutral-90"
              >
                <svg
                  width="14"
                  height="14"
            ...
            <button
              type="submit"
              className="primary-btn ml-auto color-primary-80 bg-primary-60 rounded-4 text-[14px] min-w-[60px] max-w-[80px] w-full h-[30px] font-bold"
            >
