...
import { toast_duration, toast_error_option, toast_info_option, toast_sucess_option } from "@/utils/toast";
...
export default function UserPostBox() {
    ...
    const onPostSubmit = (e: any) => {
        ...
        if (!user_inputted_image_url) return
        ...
        <>
            ...
            >
                ...
                <video ref={videoRef}
                    width={1280}
                    height={720}
                    className={`${(aspectRatio === '4:3') ? styles.ratio_4_3 : (aspectRatio === '16:9') ? styles.ratio_16_9 : styles.ratio_9_16}`}
                    style={{ transform: 'scaleX(-1)', opacity: 0.1, maxWidth: '100%' }} autoPlay muted></video>
                ...
                {isCameraModalopen && user_inputted_image_url && <section className={`${styles.confirm_photo} confirm_photo bg-neutral-90 absolute top-0 left-0 right-0 bottom-0 h-full w-full z-100`}>
                    <div
                        className={`${(aspectRatio === '4:3') ? styles.ratio_4_3 : (aspectRatio === '16:9') ? styles.ratio_16_9 : styles.ratio_9_16}`}
                    >

                        <img
                            src={user_inputted_image_url} className="w-full opacity-10" />
                        <div className={`${styles.camera_actions} absolute left-0 bottom-0 right-0 py-[40px] px-[24px] flex gap-[12px]`}>
                            <button className="primary-btn w-full h-[48px] font-bold color-primary-80 text-[16px] inline-flex justify-center items-center">Confirm</button>
                            <button className="error-btn w-full h-[48px] font-bold bg-error-80 border-error-50 color-neutral-0 text-[16px] inline-flex justify-center items-center">Confirm</button>
                        </div>
                    </div>
                </section>}
            </Modal >
