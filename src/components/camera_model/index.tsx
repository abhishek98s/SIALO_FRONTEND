...
export const CameraModel: React.FC<Props> = ({
  ...
  return (
    <>
      <Modal
        ...
        classNames={{
          modal: `camera_model ${
            isCameraModalopen ? "animate" : "animate-none"
          }`,
        }}
      >
        <Camera_Preview
        ...
        <section
          className={`${styles.ratio_wrapper} absolute top-[24px] left-[24px] flex gap-[16px]`}
        >
          <button
            onClick={() => cameraAction.toggleAspectRatio("3:4")}
            className={`${aspectRatio === "3:4" ? styles.active : ""}`}
          >
            3: 4
          </button>
          <button
            onClick={() => cameraAction.toggleAspectRatio("9:16")}
            className={`${aspectRatio === "9:16" ? styles.active : ""}`}
          >
            9: 16
          </button>
          <button
            onClick={() => cameraAction.toggleAspectRatio("1:1")}
            className={`${aspectRatio === "1:1" ? styles.active : ""}`}
          >
            1: 1
          </button>
        </section>
