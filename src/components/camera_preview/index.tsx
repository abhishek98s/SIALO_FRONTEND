...
export const Camera_Preview: React.FC<Camera_preview_props> = ({
  mediaStream,
  aspectRatio,
  setUser_inputted_image_url,
  setMediaStream,
}) => {
  ...
  return (
    <>
      ...
      <section
        className={`${styles.camera_actions} absolute left-0 bottom-0 right-0 py-[40px]`}
      >
        <div
          className={`${styles.take_photo_btn_wrapper} flex flex-col items-center gap-[16px]`}
        >
          <button
            className={`${styles.take_photo_btn} focus-visible-neutral-70 outline-neutral-90 w-[62px] h-[62px] bg-primary-45 rounded-full flex-center p-[8px]`}
            onClick={cameraAction.takePhoto}
          >
            <div
              className={`${styles.shot_photo_btn} w-full h-full bg-neutral-0 rounded-full cursor-pointer`}
            ></div>
          </button>

          <span className="color-primary-50 font-bold text-[18px]">
            Let&aposs take a photo
          </span>
        </div>
      </section>
    </>
