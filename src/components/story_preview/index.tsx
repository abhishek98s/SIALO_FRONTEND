...
export default function StoryPreview() {
  ...
  const { data: userStories, loading } = useFetchData(
    `${APP_BASE_URL}/story/${currentUserId}`,
    currentUserId
  );
  ...
  return (
    <div className="overlay fixed overflow-hidden z-[9999] flex items-center justify-center top-0 bottom-0 left-0 right-0 bg-black/60">
      <button
        ...
        className="opacity-0 lg:opacity-100 -translate-y-1/2 absolute z-10 top-1/2 bottom-0 left-0 max-w-[80px] w-full flex-center h-[80%]"
      >
        <figure>
          <Image
            loading="lazy"
      ...
      {loading && (
        <StoryLoader closeModel={closeModalCallback} styles={styles} />
      )}
      {userStories.length !== 0 && !loading && (
        <>
          <section
            ref={StoryModalRef}
            className="flex-center relative sm:w-[450px] min-h-[550px] h-[100%] sm:h-[95vh] border-neutral-80 backdrop-blur-[12px] rounded-8 overflow-hidden"
          >
            <div className="absolute top-0 left-0 right-0 z-20 flex justify-between header p-[12px]">
              <div className="flex items-center gap-[8px]">
                ...
                <div className="">
                  <div className="name text-[16px] font-bold">
                    {story.user_name}
                  </div>
                ...
                {userObjectId === story.user_id && (
                  <div className="relative">
                    <button
                      ...
                      className={`${styles.icon_button} w-[40px] h-[40px] rounded-full overflow-hidden flex-center`}
                    >
                      <Image
                        loading="lazy"
                        src="/icons/icon-more-menu.svg"
                        alt=""
                        width={24}
                        height={24}
                      />
                    ...
                    {isMenuOpen && (
                      <div
                        className={`${styles.more_btn_wrapper} absolute top-full right-0 backdrop-blur-[12px] bg-black/30 border-neutral-40 p-[2px] rounded-8`}
                      >
                        <ul className="w-[108px]">
                          <li>
                            <button
                              onClick={onDeleteClick}
                              className="w-full text-left text-[14px] p-[6px] rounded-4 color-neutral-0 focus-visible-primary-45"
                            >
            ...
            <Image
              loading="lazy"
              className="relative z-10 w-full h-auto object-contain content-center"
            ...
            />
            <div
              className={`${styles.liner_overlay} absolute top-0 bottom-0 left-0 right-0 z-4`}
            ></div>

            <div
              className={`${styles.caption} backdrop-blur-[4px] absolute bottom-0 left-0 right-0 text-center leading-[1.5] text-[16px] bg-black/20 rounded-8 p-[14px]`}
            >
              {story.caption}
            </div>
          </section>
      ...
      <button
        ref={nextRef}
        onClick={onNextClick}
        className="opacity-0 lg:opacity-100 -translate-y-1/2 absolute z-10 top-1/2 bottom-0 right-0 max-w-[80px] w-full flex-center h-[80%]"
      >
        <figure>
          <Image
            loading="lazy"
