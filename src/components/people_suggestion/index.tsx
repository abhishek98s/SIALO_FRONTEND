...
const People: React.FC<IPeople> = ({ _id, name, img, fetchPeoplList }) => {
    ...
    const sendFriendReques = async () => {
        try {
            const response = await axiosInstance.patch(`${APP_BASE_URL}/user/friend/add/${_id}`);
    ...
    return (
        <li className="flex items-center">
            <Link href={`/profile/${_id}/feed`} className="focus-visible-primary-45 rounded-4">
                <div className="flex items-center">
                    <figure className="rounded-full mr-[12px]">
                        <Image src={img ? img : '/icons/icon-user.svg'} alt={`user`} className="object-cover rounded-full border-primary-60" width={30} height={30} />
                    </figure>
            ...
            <button onClick={sendFriendReques} className="ml-auto focus-visible-primary-45 rounded-4">
