...
const SearchBox = () => {
  ...
  const { data, loading } = useFetchData(
    `${APP_BASE_URL}/user/search?=name=${encodeURIComponent(search)}`,
    search
  );
    ...
    <div className="relative">
      ...
      {search && (
        <div className="absolute left-0 right-0 top-[100%] mt-4 rounded-8 z-10 bg-neutral-88 border-neutral-80 p-2">
          ...
          {!loading &&
            data.map((people: IPeople, index: number) => (
              <Link
                key={index}
                href={`/profile/${people._id}/feed`}
                className="block rounded-4 focus-visible-primary-45"
              >
                <div
                  className={`${styles.user_wrapper} flex items-center gap-[16px] py-[12px] px-[20px]`}
                >
                  <figure className="rounded-full max-w-[40px] h-[40px]">
                    <Image
                      className="rounded-full border-primary-60 object-cover"
                      src={people.img ? people.img : "/icons/icon-user.svg"}
                      alt="user"
                      width={40}
                      height={40}
                    />
                  ...
                  <span className="color-primary-10 text-[14px]">
                    {people.name}
                  </span>
                </div>
              </Link>
          ...
          {!loading && data.length === 0 && (
            <div
              className={`${styles.user_wrapper} flex items-center gap-[16px] py-[12px] px-[20px] color-primary-10 text-[14px]`}
            >
              No result found
            </div>
          )}
